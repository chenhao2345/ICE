        ...
        for kid in indices:
            ...
            if select_cams:
                ...
                if len(select_cams) >= self.num_instances:
                    ...
                    cam_indexes = np.random.permutation(select_cams)
                    select_index = No_index(index,i)
                    select_index = np.setdiff1d(select_index, cam_indexes)
                    if len(select_index)==0:
                        ind_indexes = np.random.choice(select_cams, size=self.num_instances-1-len(cam_indexes), replace=True)
                    elif len(select_index) >= self.num_instances-1-len(cam_indexes):
                        ind_indexes = np.random.choice(select_index, size=self.num_instances-1-len(cam_indexes), replace=False)
                    else:
                        ind_indexes = np.random.choice(select_index, size=self.num_instances-1-len(cam_indexes), replace=True)
                    cam_indexes = np.concatenate((cam_indexes, ind_indexes))
                for kk in cam_indexes:
                    ret.append(index[kk])
            else:
                if len(index) >= self.num_instances:
